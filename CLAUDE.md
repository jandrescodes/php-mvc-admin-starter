...
The three HTML views (`403.php`, `404.php`, `500.php`) are minimal — each only sets `$code`, `$tone` (`'danger'` or `'warning'`), `$heading`, `$message`, then `require`s `views/layouts/error.php`, which owns the full standalone document (head, dark-mode detection, `<main>` landmark, styles). Add a new error view by following the same 5-line pattern; never inline a full `<html>` document again. Styling lives in `public/css/modules/errors/errors.css` (light tokens: `--error-bg`, `--error-danger`, `--error-warning`, etc.) and `errors-dark.css` (reuses the `--dm-*` tokens from `dark-mode.css`). The `[data-tone]` attribute on `<body>` drives which color the `.error-code` heading uses — don't hardcode colors per view.
...
**Auth standalone pages:** `views/auth/*.php` (`login`, `forgot_password`, `reset_password`, `accept_invitation`) do not use `layouts/header.php`/`footer.php`. Controllers call `Controller::renderStandalone($view, $data, $title, $module_scripts)`, which internally calls `getView()` (same output-buffering helper `render()` uses) and requires the shared `views/layouts/auth.php`. Each auth view file therefore contains **only** its page-specific card markup — no `<html>`/`<head>`, no `ob_start()`/`require` boilerplate, no manual `<script>` tags. To add a new standalone auth page: create the view with just the card HTML, call `$this->renderStandalone('auth/your_view', $data, 'Page Title', ['module/script'])` from the controller, and register the module script under `public/js/modules/auth/`. Keep input markup as `form-group > input-group` so `.invalid-feedback` placement from `common-validate.js` renders correctly, and pair every input with a `<label class="sr-only">` (placeholder alone is not an accessible label).
...
- **Standalone views (auth):** use `$this->renderStandalone($view, $data = [], $title = '', $module_scripts = [])` from `App\Core\Controller` for any page outside the admin/auth layouts (currently only `views/auth/*`). It mirrors `render()`'s `getView()` pattern and requires `views/layouts/auth.php`. Never build a full `<html>` document inside a view file — add a layout instead.
